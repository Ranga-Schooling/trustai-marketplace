...
This is the project's design and testing document. It records the
architecture and design decisions behind TrustAI Marketplace, the software
and architectural patterns used and why, the recommended deployment options
and their relative costs, and the testing strategy — what is tested, at which
layer, and the reasoning for each.

Each numbered decision (D-01, D-02, …) is a design decision taken during the
project, recorded at the point it was made. Platform-level decisions with
wider consequences are recorded separately as ADRs in
[docs/decisions/](decisions/). Testing is documented here as strategy and in
[docs/testing/README.md](testing/README.md) as procedure and evidence.
...
The full decision, the alternatives considered (managed PaaS, on-premises,
Kubernetes), and a comparison of their **relative cost implications** at both
capstone and production scale are recorded in
[ADR-003](decisions/ADR-003-aws-ec2-deployment.md). In summary:

| Option | Capstone scale | Production scale |
|---|---|---|
| AWS EC2 + ECR + SSM (**chosen**) | Free tier for 12 months, then ~$10–20/month | Add instances + load balancer; move DB to RDS (~$15–30/month more) |
| Managed PaaS (Render/Railway) | $0/month on free tiers | ~$25–50/month |
| On-premises | Electricity + static IP + a physically responsible owner | Cheapest compute, most expensive staff time |
| Kubernetes (EKS/GKE) | ~$70+/month for the control plane alone | Justified only with many services |

**Recommendation for a team without our CI/CD learning objective:** managed
PaaS. We chose EC2 because demonstrating the pipeline is itself part of the
capstone deliverable. Were this a real product, the first two infrastructure
investments should be managed Postgres (RDS) and a second application
instance behind a load balancer — in that order, because the single-instance
database is the larger risk.

Earlier planning documents recommended Render; [ADR-001](decisions/ADR-001-deployment-platform.md)
records that decision and why it was superseded.
