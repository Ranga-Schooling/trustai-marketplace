"""Offline-first CLI for the Sol/PT1 and Gemini/PT1 Capstone observations.

Only ``execute --confirm-live`` can cross a provider boundary.
"""
    ...
    *,
    ...
    try:
        ...
        if not options.confirm_live:
            raise CapstoneLiveValidationError("explicit_live_confirmation_required")
        authorization = load_strict_contract_json(options.authorization)
        ...
        record = validator.execute_one(
            authorization_document=authorization,
            confirm_live=True,
            credential_resolver=LazyEnvironmentCredentialResolver(environment_getter),
            transport=ConcreteLivePilotTransport(sender_factory()),
            operational_root=state_root,
        )
        _emit(
